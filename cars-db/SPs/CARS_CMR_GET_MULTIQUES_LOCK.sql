DROP PROCEDURE IF EXISTS CARS_CMR_GET_MULTIQUES_LOCK;
DELIMITER $$
CREATE PROCEDURE `CARS_CMR_GET_MULTIQUES_LOCK`(IN `p_moduleHdrId` INT(11),IN p_HDR_AMEND_ID INT(11), IN `p_quespagecombo_Ids` TEXT, IN `p_lockBy` VARCHAR(255), OUT `p_lockSuccessful` TINYINT(1), OUT `p_lockedBy` VARCHAR(255), OUT `p_lockedTs` TIMESTAMP, OUT `p_lockedByRoleName` VARCHAR(255))
p_loop: 
BEGIN
	...
	BEGIN
		...
		WHERE SP_LOG_ID=(SELECT MAX(SP_LOG_ID)FROM CARS_SP_LOG WHERE SP_NAME='CARS_CMR_GET_MULTIQUES_LOCK');
		COMMIT; 
	...
	INSERT INTO CARS_SP_LOG (`SP_NAME`, `START_TS`, `END_TS`)
		VALUES( 'CARS_CMR_GET_MULTIQUES_LOCK', NOW(), NOW());	
...
;	
		...
		SELECT LOCKED_BY, LOCKED_TS 
		  ...
		  FROM CARS_CMR_HDR_QUES_PAGE_LOCK
		 WHERE HDR_AMEND_ID = p_HDR_AMEND_ID 
		...
		INSERT INTO CARS_CMR_HDR_QUES_PAGE_LOCK (HDR_AMEND_ID, QUES_ID, PAGE_NUM, ROLE_NAME, LOCKED_BY, LOCKED_TS) 
			SELECT 	p_HDR_AMEND_ID, v_quesId, v_pageNum, v_roleName, p_lockBy, v_ts 
