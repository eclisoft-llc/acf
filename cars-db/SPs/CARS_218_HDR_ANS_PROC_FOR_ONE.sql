DELIMITER $$
CREATE PROCEDURE `CARS_218_HDR_ANS_PROC_FOR_ONE`(
...
BEGIN
	...
	BEGIN
		 ...
		 UPDATE CARS_SP_LOG
		  ...
		  WHERE SP_LOG_ID=(SELECT MAX(SP_LOG_ID)FROM CARS_SP_LOG WHERE SP_NAME='CARS_218_HDR_ANS_PROC_FOR_ONE');
		 COMMIT; 
	END;
	
	-- insert log record
	INSERT INTO CARS_SP_LOG (`SP_NAME`, `START_TS`, `END_TS`)
		VALUES( 'CARS_218_HDR_ANS_PROC_FOR_ONE', NOW(), NOW());	
	...
	ELSE
		...
		DELETE FROM CARS_218_HDR_ANS WHERE HDR_AMEND_ID = i_hdr_amend_id;
