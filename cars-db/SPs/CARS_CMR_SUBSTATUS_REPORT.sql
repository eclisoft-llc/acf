DROP PROCEDURE IF EXISTS CARS_CMR_SUBSTATUS_REPORT;
DELIMITER $$
CREATE PROCEDURE `CARS_CMR_SUBSTATUS_REPORT`(IN period_id INTEGER,IN excludeAcceptedFlag INTEGER, IN i_hdramend_id TEXT)
proc_label: BEGIN
	...
	BEGIN
		...
		WHERE SP_LOG_ID=(SELECT MAX(SP_LOG_ID)FROM CARS_SP_LOG WHERE SP_NAME='CARS_CMR_SUBSTATUS_REPORT');
	...
	INSERT INTO CARS_SP_LOG (SP_NAME, SP_STATUS_TEXT, START_TS)
		VALUES('CARS_CMR_SUBSTATUS_REPORT', 'Started', NOW());	
...
CC.SUBSECT_PCT AS ''Complete'',
A.INITIAL_SUBMIT_TS AS ''Initial Certified Date'',
A.SUBMIT_TS AS ''Last Certified Date'',
RR.Review_Status AS ''Review Status'',
A.READY_FOR_ACCEPTANCE_TS AS ''Ready for Acceptance Date'',
A.APPR_TS AS ''Accepted Date'',
A.APPRD_BY AS ''Accepted by''
...
LEFT JOIN (SELECT ROUND((SUM(CASE WHEN N.ANS_STATUS_TEXT IN (''COMPLETE'',''NOT STARTED'') THEN 1 ELSE 0 END) / SUM(CASE WHEN N.ANS_STATUS_TEXT IN (''VALIDATION ERROR'', ''IN PROGRESS'', ''NOT STARTED'', ''COMPLETE'') THEN 1 ELSE 0 END) ) * 100, 2) AS SUBSECT_PCT,
A.HDR_AMEND_ID  
FROM CARS_MODULE_PERIOD_HDR H
JOIN CARS_CMR_HDR_AMEND A
...
ON N.SUBQUES_RESP_ID = B.SUBQUES_RESP_ID GROUP BY A.HDR_AMEND_ID) AS CC
ON CC.HDR_AMEND_ID=A.HDR_AMEND_ID
...
'Request #' AS 'Request #','Complete' AS 'Complete','Initial Certified Date' AS 'Initial Certified Date','Last Certified Date' AS 'Last Certified Date',
'Review Status' AS 'Review Status','Ready for Acceptance Date' AS 'Ready for Acceptance Date','Accepted Date' AS 'Accepted Date','Accepted by' AS 'Accepted by';
