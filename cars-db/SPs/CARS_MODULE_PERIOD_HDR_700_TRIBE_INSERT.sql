...
DROP PROCEDURE IF EXISTS CARS_MODULE_PERIOD_HDR_700_TRIBE_INSERT;

DELIMITER $$
CREATE PROCEDURE `CARS_MODULE_PERIOD_HDR_700_TRIBE_INSERT`(
p_PERIOD_ID INT
)
BEGIN DECLARE exit handler for SQLEXCEPTION
 BEGIN
  ...
  WHERE  SP_LOG_ID=(SELECT MAX(SP_LOG_ID)FROM CARS_SP_LOG WHERE SP_NAME='CARS_MODULE_PERIOD_HDR_700_TRIBE_INSERT');
 END;
 /*Log run of the SP*/
   INSERT INTO CARS_SP_LOG(SP_NAME,START_TS,SP_STATUS_TEXT) 
  SELECT 'CARS_MODULE_PERIOD_HDR_700_TRIBE_INSERT',NOW(),'Running';
...
WHERE 1=1
AND MODULE.MODULE_NAME='ACF-700'
...
SELECT 
  ...
  JOIN CARS_TRIBE_INFO I ON I.TRIBE_ID=T.ENTITY_ID AND T.PERIOD_ID=I.PERIOD_ID 
  ...
  WHERE  S.MODULE_META_ID IS NULL
	...
	AND I.REQ_700_FLAG=1;
 
  SET v_MSG= 'Tribe and Period inserted in CARS_MODULE_PERIOD_HDR table' ;
