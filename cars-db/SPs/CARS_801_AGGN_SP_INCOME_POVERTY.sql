DELIMITER $$
CREATE OR REPLACE PROCEDURE CARS_801_AGGN_SP_INCOME_POVERTY( IN i_aggn_ref_id INT)
BEGIN
...
DECLARE v_total_rows_inserted INTEGER DEFAULT 0; 
	...
	INSERT INTO CARS_801_SP_LOG ( SP_NAME, START_TS)
		VALUES( 'CARS_801_AGGN_SP_INCOME_POVERTY', NOW());	
		
		
	SELECT COUNT(*) INTO v_rec_cnt FROM CARS_801_AGGN_INCOME_POVERTY WHERE  AGGN_REF_ID = i_aggn_ref_id;
 
		DELETE FROM CARS_801_AGGN_INCOME_POVERTY
...
DROP TEMPORARY TABLE IF EXISTS CARS_TMP_INCOME_POVERTY_INIT;
CREATE TEMPORARY TABLE CARS_TMP_INCOME_POVERTY_INIT AS  
SELECT
    H.MODULE_HDR_ID, 
	H.ENTITY_NAME, 
	H.ENTITY_ID,
...
DROP TEMPORARY TABLE IF EXISTS CARS_TMP_INCOME_POVERTY_UNADJ_DATA1;
CREATE TEMPORARY TABLE CARS_TMP_INCOME_POVERTY_UNADJ_DATA1 AS  
SELECT
...
DROP TEMPORARY TABLE IF EXISTS CARS_TMP_INCOME_POVERTY_UNADJ_DATA;
CREATE TEMPORARY TABLE CARS_TMP_INCOME_POVERTY_UNADJ_DATA AS  
SELECT
...
FROM
	...
	FROM CARS_TMP_INCOME_POVERTY_UNADJ_DATA1
	GROUP BY MODULE_HDR_ID, ENTITY_ID, ENTITY_NAME
	) A
JOIN CARS_TMP_INCOME_POVERTY_INIT I
ON A.MODULE_HDR_ID = I.MODULE_HDR_ID
;



DROP TEMPORARY TABLE IF EXISTS CARS_TMP_INCOME_POVERTY_ADJ_DATA;
CREATE TEMPORARY TABLE CARS_TMP_INCOME_POVERTY_ADJ_DATA AS  
SELECT
...
(
		...
		FROM CARS_TMP_INCOME_POVERTY_UNADJ_DATA
) A
;
		
DROP TEMPORARY TABLE IF EXISTS CARS_TMP_INCOME_POVERTY_AVG;
CREATE TEMPORARY TABLE CARS_TMP_INCOME_POVERTY_AVG AS  
SELECT
...
FROM CARS_TMP_INCOME_POVERTY_ADJ_DATA
GROUP BY ENTITY_ID, ENTITY_NAME;


DROP TEMPORARY TABLE IF EXISTS CARS_TMP_INCOME_POVERTY_NATIONAL;
CREATE TEMPORARY TABLE CARS_TMP_INCOME_POVERTY_NATIONAL AS  
SELECT
...
FROM CARS_TMP_INCOME_POVERTY_AVG;


DROP TEMPORARY TABLE IF EXISTS CARS_TMP_INCOME_POVERTY_PCT;
CREATE TEMPORARY TABLE CARS_TMP_INCOME_POVERTY_PCT AS  
SELECT
ENTITY_ID, ENTITY_NAME,
...
ROUND((INCOME_GE_175_ADJ_CNT / TOT_ADJ_CNT) * 100, 2) AS INCOME_GE_175_PCT,
100.00 AS TOT_PCT 
FROM CARS_TMP_INCOME_POVERTY_NATIONAL;

INSERT INTO CARS_801_AGGN_INCOME_POVERTY
(
AGGN_REF_ID,          
ENTITY_ID, 
ENTITY_NAME, 
...
TOT_ADJ_CNT,
TOT_PCT
FROM CARS_TMP_INCOME_POVERTY_AVG A
JOIN CARS_TMP_INCOME_POVERTY_PCT P
...
TOT_ADJ_CNT,
TOT_PCT
FROM CARS_TMP_INCOME_POVERTY_NATIONAL A
JOIN CARS_TMP_INCOME_POVERTY_PCT P
ON A.ENTITY_ID = P.ENTITY_ID
...
NULL
FROM CARS_ENTITY E
LEFT OUTER JOIN CARS_TMP_INCOME_POVERTY_AVG C
