...
CREATE OR REPLACE PROCEDURE CARS_801_AGGN_SP_MISSING_MONTHS_FOOTNOTE(IN i_aggn_ref_id INT)
...
BEGIN
   ...
   DECLARE v_total_rows_inserted INTEGER DEFAULT 0; 
	...
	INSERT INTO CARS_801_SP_LOG ( SP_NAME, START_TS)
		VALUES( 'CARS_801_AGGN_SP_MISSING_MONTHS_FOOTNOTE', NOW());	
		
		DELETE FROM CARS_801_AGGN_REPORT_FNOTE
		WHERE AGGN_REF_ID = i_aggn_ref_id AND REPORT_NUM = 'ALL' AND FNOTE_SEQ_NUM = 1;
        
		SET v_total_rows_deleted = ROW_COUNT();

SET v_FISCAL_YEAR=(SELECT FISCAL_YEAR FROM CARS_801_AGGN_REF WHERE AGGN_REF_ID = i_aggn_ref_id);
