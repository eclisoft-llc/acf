DROP PROCEDURE IF EXISTS `CARS_118_FULLPLAN_SUBSTATUS_REPORT`;
DELIMITER $$
CREATE PROCEDURE `CARS_118_FULLPLAN_SUBSTATUS_REPORT`(IN i_amend_where_cond TEXT,IN i_sectid_where_cond TEXT)
proc_label: BEGIN
...
DECLARE v_crsql TEXT DEFAULT '';
	...
	BEGIN
		...
		WHERE SP_LOG_ID=(SELECT MAX(SP_LOG_ID)FROM CARS_SP_LOG WHERE SP_NAME='CARS_118_FULLPLAN_SUBSTATUS_REPORT');
	...
	INSERT INTO CARS_SP_LOG (SP_NAME, SP_STATUS_TEXT, START_TS)
		VALUES('CARS_118_FULLPLAN_SUBSTATUS_REPORT', 'Started', NOW());	
...
JOIN CARS_118_SUBSECT SS
ON S.SECT_ID = SS.SECT_ID
WHERE S.SECT_ID IN (', i_sectid_where_cond, ')
'
)
;
...
( ENTITY_NAME TEXT NULL, STATE_CD TEXT NULL, REGION_NAME TEXT NULL, REGION_ID INT NULL,AMENDMENT_NUMBER TEXT NULL, ');
...
( ENTITY_NAME TEXT NULL, STATE_CD TEXT NULL, REGION_NAME TEXT NULL, REGION_ID TEXT NULL,AMENDMENT_NUMBER TEXT NULL, ');
...
AS 
SELECT
DISTINCT H.ENTITY_NAME, A.HDR_AMEND_ID, H.ENTITY_STATE_CD AS STATE_CD, R.ENTITY_NAME AS REGION_NAME, E.REGION_ID AS REGION_ID,
...
JOIN CARS_118_SUBQUES SQ
ON R.SUBQUES_ID = SQ.SUBQUES_ID
JOIN CARS_118_QUES Q
ON SQ.QUES_ID = Q.QUES_ID
JOIN CARS_118_SUBSECT S
...
SELECT
			...
			ELSE NULL END AS SUBSECT_STATUS
				...
				(
					...
					FROM VW_CARS_118_HDR_QUES_STATUS S
					WHERE SECT_ID IN (', i_sectid_where_cond, ')
					GROUP BY 1, 2
				) B
'
);
...
b_while:
	WHILE v_whilecnt <= v_iter DO
		...
		SET v_sql = CONCAT(
		'INSERT INTO CARS_TEMP_SUBSECT_STATUS_COLS', v_rand, ' ( ENTITY_NAME, STATE_CD, REGION_NAME, REGION_ID,AMENDMENT_NUMBER, COL', v_whilecnt, 
		') 
		SELECT
		M.ENTITY_NAME, STATE_CD, REGION_NAME, REGION_ID, AMENDMENT_NUMBER,
		...
		' GROUP BY ENTITY_NAME, STATE_CD, REGION_NAME, REGION_ID'
		)
		;
...
SET v_sql = CONCAT(
		'INSERT INTO CARS_TEMP_SUBSECT_STATUS_COLS', v_rand, ' (ENTITY_NAME, STATE_CD, REGION_NAME, REGION_ID,AMENDMENT_NUMBER)
		SELECT M.ENTITY_NAME, M.STATE_CD, M.REGION_NAME, M.REGION_ID,M.AMENDMENT_NUMBER
		FROM CARS_TEMP_SUBSECT_AMEND_LIST', v_rand, ' M
...
b1_while:
	WHILE v_whilecnt <= v_iter DO
		...
		SET v_sql = CONCAT(
		'INSERT INTO CARS_TEMP_SUBSECT_HEADER_COLS', v_rand, ' ( ENTITY_NAME, STATE_CD, REGION_NAME, REGION_ID,AMENDMENT_NUMBER, HEADER', v_whilecnt, 
		') 
		SELECT
		''HEADER ROW'', NULL, NULL, NULL, NULL, 
		SUBSECT_NAV_tEXT
...
SET v_sqlhdr = CONCAT('INSERT INTO CARS_TEMP_SUBSECT_HEADERS', v_rand, ' SELECT ''Lead Agency'', ''State'', ''Region'', ''REGION_ID'',''Plan Version'', ');
SET v_sqlans = CONCAT('INSERT INTO CARS_TEMP_SUBSECT_STATUS', v_rand,' SELECT ENTITY_NAME, STATE_CD, REGION_NAME, REGION_ID, AMENDMENT_NUMBER, ');
...
SET v_sqlhdr = CONCAT(v_sqlhdr, ' FROM CARS_TEMP_SUBSECT_HEADER_COLS', v_rand, ' GROUP BY ENTITY_NAME, STATE_CD, REGION_NAME, REGION_ID, AMENDMENT_NUMBER');
SET v_sqlans = CONCAT(v_sqlans, ' FROM CARS_TEMP_SUBSECT_STATUS_COLS', v_rand, ' GROUP BY ENTITY_NAME, STATE_CD, REGION_NAME, REGION_ID, AMENDMENT_NUMBER');
