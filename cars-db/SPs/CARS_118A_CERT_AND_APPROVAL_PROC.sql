DELIMITER $$
CREATE OR REPLACE PROCEDURE `CARS_118A_CERT_AND_APPROVAL_PROC`(IN period_id INTEGER,IN amend_id TEXT)
proc_label: BEGIN
	...
	BEGIN
		...
		WHERE SP_LOG_ID=(SELECT MAX(SP_LOG_ID)FROM CARS_SP_LOG WHERE SP_NAME='CARS_118A_CERT_AND_APPROVAL_PROC');
			
		
	END;
	
		INSERT INTO CARS_SP_LOG (SP_NAME, SP_STATUS_TEXT, START_TS)
		VALUES('CARS_118A_CERT_AND_APPROVAL_PROC', 'Started', NOW());	
...
AND R.ENTITY_TYPE_CD = 'REGION'
	...
	LEFT JOIN (SELECT HDR_AMEND_ID,COUNT(1) AS CNT FROM CARS_118A_SUBQUES_AMENDMENT WHERE IS_AMENDED=1 
...
GROUP BY ha.HDR_AMEND_ID) AS AQ ON AQ.HDR_AMEND_ID=B.HDR_AMEND_ID
	...
	LEFT JOIN CARS_118A_HDR_APPROVAL_LETTER AS P ON P.HDR_AMEND_ID=B.HDR_AMEND_ID
			...
			WHERE IFNULL(T.PROGRAM_477_FLAG, 0) <> 1 
				AND B.HDR_AMEND_ID IN ((select TRIM(j.name) AS HDR_AMEND_ID
		from json_table(
	...
	SELECT 'Lead Agency' AS 'Lead Agency', 'Tribal Code' AS 'Tribal Code', 'State' AS 'State','Region' AS 'Region','Region_ID' AS 'Region_ID','Allocation' AS 'Allocation','Plan Version' AS 'Plan Version','Initial Certified Date' AS 'Initial Certified Date',
	...
	'Ready for Signature Date' AS 'Ready for Signature Date', 'Approval Date' AS 'Approval Date';
...
JOIN CARS_MODULE_PERIOD_HDR H ON H.MODULE_HDR_ID = B.MODULE_HDR_ID AND H.PERIOD_ID=period_id
JOIN CARS_TRIBE_INFO T
ON B.PERIOD_ID = T.PERIOD_ID
AND B.ENTITY_ID = T.TRIBE_ID
JOIN CARS_ENTITY E
