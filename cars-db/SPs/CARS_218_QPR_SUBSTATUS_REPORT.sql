DELIMITER $$
CREATE OR REPLACE PROCEDURE `CARS_218_QPR_SUBSTATUS_REPORT`(IN i_amend_where_cond TEXT, IN i_period_id INTEGER, IN i_sect_id TEXT)
proc_label: 
...
DECLARE v_crsql TEXT DEFAULT '';
	...
	BEGIN
		...
		WHERE SP_LOG_ID=(SELECT MAX(SP_LOG_ID)FROM CARS_SP_LOG WHERE SP_NAME='CARS_218_QPR_SUBSTATUS_REPORT');
	...
	INSERT INTO CARS_SP_LOG (SP_NAME, SP_STATUS_TEXT, START_TS)
		VALUES('CARS_218_QPR_SUBSTATUS_REPORT', 'Started', NOW());	
...
( ENTITY_NAME TEXT NULL, STATE_CD TEXT NULL, REGION_NAME TEXT NULL, REGION_ID INT NULL, ');
...
( ENTITY_NAME TEXT NULL, STATE_CD TEXT NULL, REGION_NAME TEXT NULL, REGION_ID TEXT NULL, ');
...
WHERE H.ENTITY_ID IN (', i_amend_where_cond, ')')
;
...
SELECT
			...
			ELSE NULL END AS SUBSECT_STATUS
				...
				(
					...
					FROM VW_CARS_218_HDR_QUES_STATUS S
					WHERE SECT_ID IN(',i_sect_id,')
					GROUP BY 1, 2
				) B
'
);
...
b_while:
	WHILE v_whilecnt <= v_iter DO
		...
		SET v_sql = CONCAT(
		'INSERT INTO CARS_TEMP_SUBSECT_STATUS_COLS', v_rand, ' ( ENTITY_NAME, STATE_CD, REGION_NAME, REGION_ID, COL', v_whilecnt, 
		') 
		SELECT
		M.ENTITY_NAME, M.STATE_CD, M.REGION_NAME, M.REGION_ID,  
...
SET v_sql = CONCAT(
		'INSERT INTO CARS_TEMP_SUBSECT_STATUS_COLS', v_rand, ' (ENTITY_NAME, STATE_CD, REGION_NAME, REGION_ID)
		SELECT M.ENTITY_NAME, M.STATE_CD, M.REGION_NAME, M.REGION_ID
		FROM CARS_TEMP_SUBSECT_AMEND_LIST', v_rand, ' M
...
b1_while:
	WHILE v_whilecnt <= v_iter DO
		...
		SET v_sql = CONCAT(
		'INSERT INTO CARS_TEMP_SUBSECT_HEADER_COLS', v_rand, ' ( ENTITY_NAME, STATE_CD, REGION_NAME, REGION_ID, HEADER', v_whilecnt, 
		') 
		SELECT
...
SET v_sqlhdr = CONCAT('INSERT INTO CARS_TEMP_SUBSECT_HEADERS', v_rand, ' SELECT ''Lead Agency'', ''State'', ''Region'', ''REGION_ID'', ');
SET v_sqlans = CONCAT('INSERT INTO CARS_TEMP_SUBSECT_STATUS', v_rand,' SELECT ENTITY_NAME, STATE_CD, REGION_NAME, REGION_ID, ');
...
SET v_sqlhdr = CONCAT(v_sqlhdr, ' FROM CARS_TEMP_SUBSECT_HEADER_COLS', v_rand, ' GROUP BY ENTITY_NAME, STATE_CD, REGION_NAME, REGION_ID');
SET v_sqlans = CONCAT(v_sqlans, ' FROM CARS_TEMP_SUBSECT_STATUS_COLS', v_rand, ' GROUP BY ENTITY_NAME, STATE_CD, REGION_NAME, REGION_ID');
