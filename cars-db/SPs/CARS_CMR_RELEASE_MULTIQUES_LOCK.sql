DROP PROCEDURE IF EXISTS CARS_CMR_RELEASE_MULTIQUES_LOCK;
DELIMITER $$
CREATE PROCEDURE `CARS_CMR_RELEASE_MULTIQUES_LOCK`(
	IN p_moduleHdrId INT(11), 
	IN p_HDR_AMEND_ID INT(11),
...
BEGIN
	...
	BEGIN
		...
		WHERE SP_LOG_ID=(SELECT MAX(SP_LOG_ID)FROM CARS_SP_LOG WHERE SP_NAME='CARS_CMR_RELEASE_MULTIQUES_LOCK');
		COMMIT; 
	...
	INSERT INTO CARS_SP_LOG (`SP_NAME`, `START_TS`, `END_TS`)
		VALUES( 'CARS_CMR_RELEASE_MULTIQUES_LOCK', NOW(), NOW());	
	...
	WHILE v_leave <> 1 DO	
		...
		DELETE FROM CARS_CMR_HDR_QUES_PAGE_LOCK
		WHERE
			HDR_AMEND_ID = p_HDR_AMEND_ID 
